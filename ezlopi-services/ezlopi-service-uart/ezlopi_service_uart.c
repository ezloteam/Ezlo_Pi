...
static void serial_init(void)
{
    ...
    uart_parity_t parity_val = UART_PARITY_DISABLE;
    uart_hw_flowcontrol_t flow_control_val = UART_HW_FLOWCTRL_DISABLE;
    ...
    flow_control_val = (false == flow_control) ? UART_HW_FLOWCTRL_DISABLE : UART_HW_FLOWCTRL_CTS_RTS;
...
static int ezlopi_service_uart_read_uart_params(const cJSON* root)
{
    ...
    CJSON_GET_VALUE_BOOL(root, ezlopi_flow_control_str, flow_control);
    ...
    EZPI_CORE_nvs_write_flow_control(flow_control);
...
{
    int ret = 0;
    cJSON* cj_firmware_info = cJSON_AddObjectToObject(parent, "ezlopi_firmware_version");
    if (cj_firmware_info)
    {
        cJSON_AddStringToObject(cj_firmware_info, ezlopi_version_str, VERSION_STR);
        cJSON_AddNumberToObject(cj_firmware_info, ezlopi_firmware_build_str, BUILD);
        cJSON_AddNumberToObject(cj_firmware_info, ezlopi_build_date_str, BUILD_DATE);
        cJSON_AddStringToObject(cj_firmware_info, ezlopi_dev_flash_str, CONFIG_ESPTOOLPY_FLASHSIZE);
        ret = 1;
    }
    return ret;
}
...
{
    int ret = 0;
    cJSON* cj_device_chip_info = cJSON_AddObjectToObject(parent, "device_chip_info");
    if (cj_device_chip_info)
    {
        esp_chip_info_t chip_info;
        char chip_revision[10];
        esp_chip_info(&chip_info);
        sprintf(chip_revision, "%.2f", (float)(chip_info.full_revision / 100.0));
        cJSON_AddStringToObject(cj_device_chip_info, "chip_type", ezlopi_chip_type_str(chip_info.model));
        cJSON_AddStringToObject(cj_device_chip_info, "chip_version", chip_revision);
        ret = 1;
    }
    return ret;
}
...
{
    int ret = 0;
    cJSON* cj_firmware_sdk_info = cJSON_AddObjectToObject(parent, "firmware_sdk_info");
    if (cj_firmware_sdk_info)
    {
        cJSON_AddStringToObject(cj_firmware_sdk_info, "firmware_SDK_name", "ESP-IDF");
        cJSON_AddStringToObject(cj_firmware_sdk_info, "firmware_SDK_version", esp_get_idf_version());
        ret = 1;
    }
    return ret;
}
...
{
    int ret = 0;
    cJSON* cj_device_serial_protocol_info = cJSON_AddObjectToObject(parent, "serial_device_protocol_info");
    if (cj_device_serial_protocol_info)
    {
        uint32_t baud = 0;
        uint8_t start_bits = 0;
        uint8_t stop_bits = 0;
        ...
        EZPI_CORE_nvs_read_baud(&baud);
        cJSON_AddNumberToObject(cj_device_serial_protocol_info, ezlopi_baud_rate_str, baud);

        EZPI_CORE_nvs_read_parity((uint8_t*)&parity_val);
        char parity = get_parity(parity_val);
        cJSON_AddStringToObject(cj_device_serial_protocol_info, "parity", &parity);

        EZPI_CORE_nvs_read_start_bits(&start_bits);
        cJSON_AddNumberToObject(cj_device_serial_protocol_info, "start_bits", start_bits);

        EZPI_CORE_nvs_read_stop_bits(&stop_bits);
        cJSON_AddNumberToObject(cj_device_serial_protocol_info, "stop_bits", stop_bits);

        EZPI_CORE_nvs_read_frame_size(&frame_size);
        cJSON_AddNumberToObject(cj_device_serial_protocol_info, "data_bits", frame_size);

        EZPI_CORE_nvs_read_flow_control(&flow_control);
        cJSON_AddNumberToObject(cj_device_serial_protocol_info, "flow_control", flow_control);
...
{
    int ret = 0;
    cJSON* cj_cj_device_ezlopi_info = cJSON_AddObjectToObject(parent, "device_ezlopi_info");
    if (cj_cj_device_ezlopi_info)
    {
        ...
        cJSON_AddStringToObject(cj_cj_device_ezlopi_info, ezlopi_uuid_str, controller_uuid ? controller_uuid : "");
        cJSON_AddStringToObject(cj_cj_device_ezlopi_info, ezlopi_uuid_prov_str, provisioning_uuid ? provisioning_uuid : "");
        cJSON_AddStringToObject(cj_cj_device_ezlopi_info, ezlopi_ezlopi_device_type_str, device_type ? device_type : "");
...
{
    int ret = 0;
    cJSON* cj_device_oem_info = cJSON_AddObjectToObject(parent, "device_oem_info");
    if (cj_device_oem_info)
    {
        ...
        cJSON_AddStringToObject(cj_device_oem_info, ezlopi_brand_str, device_brand ? device_brand : "");
        cJSON_AddStringToObject(cj_device_oem_info, ezlopi_manf_name_str, device_manufacturer ? device_manufacturer : "");
        cJSON_AddStringToObject(cj_device_oem_info, ezlopi_model_num_str, device_model ? device_model : "");
...
{
    int ret = 0;
    cJSON* cj_device_network_info = cJSON_AddObjectToObject(parent, "device_network_info");
    if (cj_device_network_info)
    {
        char* wifi_ssid = ezlopi_factory_info_v3_get_ssid();
        cJSON_AddStringToObject(cj_device_network_info, ezlopi_ssid_str, wifi_ssid ? wifi_ssid : "");
        ...
        if (wifi_status)
        {
            ...
            cJSON_AddStringToObject(cj_device_network_info, "wifi_mode", wifi_mode ? wifi_mode : "");
            cJSON_AddBoolToObject(cj_device_network_info, "wifi_connection_status", wifi_status->wifi_connection);

            char ip_str[20];
            memset(ip_str, 0, 20);
            snprintf(ip_str, 20, IPSTR, IP2STR(&wifi_status->ip_info->ip));
            cJSON_AddStringToObject(cj_device_network_info, "ip_sta", ip_str);

            memset(ip_str, 0, 20);
            snprintf(ip_str, 20, IPSTR, IP2STR(&wifi_status->ip_info->netmask));
            cJSON_AddStringToObject(cj_device_network_info, "ip_nmask", ip_str);

            memset(ip_str, 0, 20);
            snprintf(ip_str, 20, IPSTR, IP2STR(&wifi_status->ip_info->gw));
            cJSON_AddStringToObject(cj_device_network_info, "ip_gw", ip_str);
            ...
            cJSON_AddBoolToObject(cj_device_network_info, "cloud_connection_status", cloud_connection_status);
...
static void ezlopi_service_uart_get_info()
{
    ...
    if (cj_get_info)
    {
        ...
        get_ezlopi_firmware_info(cj_get_info);
        get_ezlopi_chip_info(cj_get_info);
        get_firmware_sdk_info(cj_get_info);
        get_device_status(cj_get_info);
        get_device_serial_protocol_info(cj_get_info);
        get_ezlopi_device_ezlopi_info(cj_get_info);
        get_ezlopi_device_oem_info(cj_get_info);
        get_ezlopi_device_newtwork_info(cj_get_info);
