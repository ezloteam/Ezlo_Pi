 ...
 * @return int
 */
int sensor_0012_bme280_sensor_v3(e_ezlopi_actions_t action, l_ezlopi_item_t *item, void *arg, void *user_arg)
{
    switch (action)
    {
...
static int __prepare(void *arg)
{
    ...
    if (prep_arg && prep_arg->cjson_device)
    {
        ...
        if (temp_humid_device && pressure_deivce)
        {
            ...
            l_ezlopi_item_t *temperature_item = ezlopi_device_add_item_to_device(temp_humid_device, sensor_0012_bme280_sensor_v3);
            if (temperature_item)
            {
                __prepare_temperature_properties(temperature_item, prep_arg->cjson_device);
            }
            l_ezlopi_item_t *humidity_item = ezlopi_device_add_item_to_device(temp_humid_device, sensor_0012_bme280_sensor_v3);
            if (humidity_item)
            {
            ...
            l_ezlopi_item_t *pressure_item = ezlopi_device_add_item_to_device(pressure_deivce, sensor_0012_bme280_sensor_v3);
            if (pressure_item)
            {
