...
static int __0048_prepare(void *arg)
{
    ...
    {
        //---------------------------  DIGI - DEVICE 1 --------------------------------------------
        l_ezlopi_device_t *MQ4_device_digi = ezlopi_device_add_device(device_prep_arg->cjson_device);
        if (MQ4_device_digi)
        {
        ...
        mq4_value_t *MQ4_value = (mq4_value_t *)malloc(sizeof(mq4_value_t));
        if (NULL != MQ4_value)
        {
            memset(MQ4_value, 0, sizeof(mq4_value_t));
            l_ezlopi_device_t *MQ4_device_adc = ezlopi_device_add_device(device_prep_arg->cjson_device);
            if (MQ4_device_adc)
            {
