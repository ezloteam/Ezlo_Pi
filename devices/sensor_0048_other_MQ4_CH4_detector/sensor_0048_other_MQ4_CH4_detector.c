...
static float Extract_MQ4_sensor_ppm(uint32_t mq4_adc_pin);
...
static int __0048_get_item(l_ezlopi_item_t *item, void *arg)
{
    ...
    if (item && arg)
    {
        ...
        if (cj_result)
        {
            ...
            {
                char *valueFormatted = ezlopi_valueformatter_float(_CH4_ppm);
