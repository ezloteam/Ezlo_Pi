...
#define ADD_PROPERTIES_DEVICE_LIST(device_id, category, subcategory, item_name, value_type, cjson_device)                     \
    {                                                                                                                         \
        s_ezlopi_device_properties_t *_properties = sensor_other_mq7_prepare_properties(device_id, category, subcategory,     \
...
//                          Declaration
//*************************************************************************
static uint8_t mq7_digital_pin = 0;
static uint8_t mq7_adc_pin = 0;
...
static int add_device_to_list(s_ezlopi_prep_arg_t *device_prep_args, s_ezlopi_device_properties_t *sensor_other_mq7_properties, void *user_arg);

static s_ezlopi_device_properties_t *sensor_other_mq7_prepare_properties(uint32_t DEVICE_ID, const char *CATEGORY, const char *SUB_CATEGORY, const char *ITEM_NAME, const char *VALUE_TYPE, cJSON *cjson_device); // you can directly add the prepare args here
static int sensor_other_MQ7_prepare_and_add(void *arg);
static int sensor_other_MQ7_init(s_ezlopi_device_properties_t *properties);
static void sensor_other_MQ7_get_item(s_ezlopi_device_properties_t *properties, void *arg);
static int sensor_other_MQ7_get_value(s_ezlopi_device_properties_t *properties, void *arg);
static void Extract_MQ7_sensor_ppm(float *analog_sensor_volt, float *_ppm, s_ezlopi_device_properties_t *properties);
//--------------------------------------------------------------------------------------------------------------------------------------
int sensor_0062_MQ7_CO(e_ezlopi_actions_t action, s_ezlopi_device_properties_t *ezlopi_device, void *arg, void *user_args)
{
    int ret = 0;
    switch (action)
    {
    ...
    {
        ret = sensor_other_MQ7_prepare_and_add(arg);
        break;
    }
    case EZLOPI_ACTION_INITIALIZE:
    {
        ret = sensor_other_MQ7_init(ezlopi_device);
        break;
    }
    case EZLOPI_ACTION_HUB_GET_ITEM:
    {
        sensor_other_MQ7_get_item(ezlopi_device, arg);
        break;
    }
    case EZLOPI_ACTION_GET_EZLOPI_VALUE:
    {
        ret = sensor_other_MQ7_get_value(ezlopi_device, arg);
        break;
    }
...
static s_ezlopi_device_properties_t *sensor_other_mq7_prepare_properties(uint32_t DEVICE_ID, const char *CATEGORY, const char *SUB_CATEGORY, const char *ITEM_NAME, const char *VALUE_TYPE, cJSON *cjson_device)
{
    s_ezlopi_device_properties_t *sensor_0062_other_MQ7_properties = NULL;
    if (NULL != cjson_device)
    {
        sensor_0062_other_MQ7_properties = (s_ezlopi_device_properties_t *)malloc(sizeof(s_ezlopi_device_properties_t));
        if (sensor_0062_other_MQ7_properties)
        {
            memset(sensor_0062_other_MQ7_properties, 0, sizeof(s_ezlopi_device_properties_t));
            sensor_0062_other_MQ7_properties->interface_type = EZLOPI_DEVICE_INTERFACE_ANALOG_INPUT;
            ...
            ASSIGN_DEVICE_NAME(sensor_0062_other_MQ7_properties, device_name);
            sensor_0062_other_MQ7_properties->ezlopi_cloud.category = CATEGORY;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.subcategory = SUB_CATEGORY;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.item_name = ITEM_NAME;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.device_type = dev_type_sensor;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.value_type = VALUE_TYPE;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.has_getter = true;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.has_setter = false;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.reachable = true;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.battery_powered = false;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.show = true;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.room_name[0] = '\0';
            sensor_0062_other_MQ7_properties->ezlopi_cloud.device_id = DEVICE_ID;
            sensor_0062_other_MQ7_properties->ezlopi_cloud.room_id = ezlopi_cloud_generate_room_id();
            sensor_0062_other_MQ7_properties->ezlopi_cloud.item_id = ezlopi_cloud_generate_item_id();
            if (ezlopi_item_name_gas_alarm == ITEM_NAME)
            {
                CJSON_GET_VALUE_INT(cjson_device, "gpio1", mq7_digital_pin);
                TRACE_I("MQ7-> DIGITAL_PIN: %d ", mq7_digital_pin);
            }
            if (ezlopi_item_name_smoke_density == ITEM_NAME)
            {
                CJSON_GET_VALUE_INT(cjson_device, "gpio2", mq7_adc_pin);
                TRACE_I("MQ7-> ADC_PIN: %d ", mq7_adc_pin);
            }
            sensor_0062_other_MQ7_properties->interface.adc.resln_bit = 3; // ADC 12-bit
        }
    }
    return sensor_0062_other_MQ7_properties;
}

static int sensor_other_MQ7_prepare_and_add(void *arg) // carries cJSON
{
    int ret = 0;
...
{
    int ret = 0;
    if (sensor_other_mq7_properties)
    {
        if (0 == ezlopi_devices_list_add(device_prep_args->device, sensor_other_mq7_properties, NULL))
        {
            free(sensor_other_mq7_properties);
        }
        else
        {
            ret = 1;
...
static int sensor_other_MQ7_init(s_ezlopi_device_properties_t *properties)
{
    ...
    if ((!guard_digi) && (0 != mq7_digital_pin) && GPIO_IS_VALID_GPIO(mq7_digital_pin))
    { // intialize digital_pin
        gpio_config_t input_conf = {};
        input_conf.pin_bit_mask = (1ULL << (mq7_digital_pin));
    ...
    if ((!guard_adc) && (0 != mq7_adc_pin) && GPIO_IS_VALID_GPIO(mq7_adc_pin))
    {
        // initialize analog_pin
        ezlopi_adc_init(mq7_adc_pin, properties->interface.adc.resln_bit);
        guard_adc = true;
        ...
        if (!Calibration_complete)
        {
            xTaskCreate(Calibrate_MQ7_R0_resistance, "Task_to_calculate_R0_air", 2048, (void *)properties, 1, NULL);
        }
        ret = 1;
    }
...
static void Extract_MQ7_sensor_ppm(float *analog_sensor_volt, float *_ppm, s_ezlopi_device_properties_t *properties)
{
    ...
    *analog_sensor_volt = *analog_sensor_volt / 10.0f;
    ...
    //-----------------------------------------------------------------------------------
    // Stage_2 : [from 'sensor_0062_other_MQ7_CO_gas_detector.h']
    ...
    // 1.2 Calculate _ppm
    *_ppm = (float)pow(10, (((float)log10(_ratio)) - b_coeff_mq7) / m_slope_mq7); // ---> _ppm = 10 ^ [ ( log(ratio) - b ) / m ]
    if (*_ppm < 0)
    {
...
static void sensor_other_MQ7_get_item(s_ezlopi_device_properties_t *properties, void *arg)
{
    ...
    if (cjson_properties)
    {
        ...
        if (ezlopi_item_name_smoke_density == properties->ezlopi_cloud.item_name)
        {
            ...
            Extract_MQ7_sensor_ppm(&analog_sensor_volt, &_ppm, properties);
