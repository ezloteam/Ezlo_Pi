...
#define ADD_PROPERTIES_DEVICE_LIST(device_id, category, subcategory, item_name, value_type, cjson_device)                     \
    {                                                                                                                         \
        s_ezlopi_device_properties_t *_properties = sensor_other_mq9_prepare_properties(device_id, category, subcategory,     \
...
//                          Declaration
//*************************************************************************
static uint8_t mq9_digital_pin = 0;
static uint8_t mq9_adc_pin = 0;
...
static int add_device_to_list(s_ezlopi_prep_arg_t *device_prep_args, s_ezlopi_device_properties_t *sensor_other_mq9_properties, void *user_arg);

static s_ezlopi_device_properties_t *sensor_other_mq9_prepare_properties(uint32_t DEVICE_ID, const char *CATEGORY, const char *SUB_CATEGORY, const char *ITEM_NAME, const char *VALUE_TYPE, cJSON *cjson_device); // you can directly add the prepare args here
static int sensor_other_MQ9_prepare_and_add(void *arg);
static int sensor_other_MQ9_init(s_ezlopi_device_properties_t *properties);
...
int sensor_0063_MQ9_LPG_flameable(e_ezlopi_actions_t action, s_ezlopi_device_properties_t *ezlopi_device, void *arg, void *user_args)
{
    ...
    {
        ret = sensor_other_MQ9_prepare_and_add(arg);
        break;
    }
    case EZLOPI_ACTION_INITIALIZE:
    {
        ret = sensor_other_MQ9_init(ezlopi_device);
        break;
    }
    case EZLOPI_ACTION_HUB_GET_ITEM:
    {
        sensor_other_MQ9_get_item(ezlopi_device, arg);
        break;
    }
    case EZLOPI_ACTION_GET_EZLOPI_VALUE:
    {
        ret = sensor_other_MQ9_get_value(ezlopi_device, arg);
        break;
    }
...
static s_ezlopi_device_properties_t *sensor_other_mq9_prepare_properties(uint32_t DEVICE_ID, const char *CATEGORY, const char *SUB_CATEGORY, const char *ITEM_NAME, const char *VALUE_TYPE, cJSON *cjson_device)
{
    s_ezlopi_device_properties_t *sensor_0063_other_MQ9_properties = NULL;
    if (NULL != cjson_device)
    {
        sensor_0063_other_MQ9_properties = (s_ezlopi_device_properties_t *)malloc(sizeof(s_ezlopi_device_properties_t));
        if (sensor_0063_other_MQ9_properties)
        {
            memset(sensor_0063_other_MQ9_properties, 0, sizeof(s_ezlopi_device_properties_t));
            sensor_0063_other_MQ9_properties->interface_type = EZLOPI_DEVICE_INTERFACE_ANALOG_INPUT;

            // set the device name according to device_id
            char *device_name = NULL;
            if (ezlopi_item_name_gas_alarm == ITEM_NAME)
            {
                device_name = "MQ9-LPG_CH4-alert";
            }
            if (ezlopi_item_name_smoke_density == ITEM_NAME)
            {
                device_name = "MQ9-LPG_CH4-level[ppm]";
            }
            CJSON_GET_VALUE_STRING(cjson_device, "dev_name", device_name);

            ASSIGN_DEVICE_NAME(sensor_0063_other_MQ9_properties, device_name);
            sensor_0063_other_MQ9_properties->ezlopi_cloud.category = CATEGORY;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.subcategory = SUB_CATEGORY;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.item_name = ITEM_NAME;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.device_type = dev_type_sensor;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.value_type = VALUE_TYPE;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.has_getter = true;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.has_setter = false;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.reachable = true;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.battery_powered = false;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.show = true;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.room_name[0] = '\0';
            sensor_0063_other_MQ9_properties->ezlopi_cloud.device_id = DEVICE_ID;
            sensor_0063_other_MQ9_properties->ezlopi_cloud.room_id = ezlopi_cloud_generate_room_id();
            sensor_0063_other_MQ9_properties->ezlopi_cloud.item_id = ezlopi_cloud_generate_item_id();
            if (ezlopi_item_name_gas_alarm == ITEM_NAME)
            {
                CJSON_GET_VALUE_INT(cjson_device, "gpio1", mq9_digital_pin);
                TRACE_I("MQ9-> DIGITAL_PIN: %d ", mq9_digital_pin);
            }
            if (ezlopi_item_name_smoke_density == ITEM_NAME)
            {
                CJSON_GET_VALUE_INT(cjson_device, "gpio2", mq9_adc_pin);
                TRACE_I("MQ9-> ADC_PIN: %d ", mq9_adc_pin);
            }
            sensor_0063_other_MQ9_properties->interface.adc.resln_bit = 3; // ADC 12-bit
        }
    }
    return sensor_0063_other_MQ9_properties;
}

static int sensor_other_MQ9_prepare_and_add(void *arg) // carries cJSON
{
    int ret = 0;
...
{
    int ret = 0;
    if (sensor_other_mq9_properties)
    {
        if (0 == ezlopi_devices_list_add(device_prep_args->device, sensor_other_mq9_properties, NULL))
        {
            free(sensor_other_mq9_properties);
        }
        else
        {
            ret = 1;
...
void Calibrate_MQ9_R0_resistance(void *params)
{
    ...
    MQ9_R0_constant = (RS_calib / RatioMQ9CleanAir); // Calculate MQ9_R0_constant
    TRACE_E("CALIB_TASK -> 'MQ9_R0_constant' = %.2f", MQ9_R0_constant);
    if (MQ9_R0_constant < 0)
    {
        MQ9_R0_constant = 0; // No negative values accepted.
    }
...
static int sensor_other_MQ9_init(s_ezlopi_device_properties_t *properties)
{
    ...
    if ((!guard_digi) && (0 != mq9_digital_pin) && GPIO_IS_VALID_GPIO(mq9_digital_pin))
    { // intialize digital_pin
        gpio_config_t input_conf = {};
        input_conf.pin_bit_mask = (1ULL << (mq9_digital_pin));
    ...
    if ((!guard_adc) && (0 != mq9_adc_pin) && GPIO_IS_VALID_GPIO(mq9_adc_pin))
    {
        // initialize analog_pin
        ezlopi_adc_init(mq9_adc_pin, properties->interface.adc.resln_bit);
        guard_adc = true;
        ...
        if (!Calibration_complete)
        {
            xTaskCreate(Calibrate_MQ9_R0_resistance, "Task_to_calculate_R0_air", 2048, (void *)properties, 1, NULL);
        }
        ret = 1;
    }
...
static void Extract_MQ9_sensor_ppm(float *analog_sensor_volt, float *_ppm, s_ezlopi_device_properties_t *properties)
{
    ...
    *analog_sensor_volt = *analog_sensor_volt / 10.0f;
    ...
    //-----------------------------------------------------------------------------------
    // Stage_2 : [from 'sensor_0063_other_MQ9_LPG_CH4_flamable_detector.h']
    ...
    // 1.1 Calculate @ 'ratio' during LPG_CH4 presence
    double _ratio = (Rs_gas / ((MQ9_R0_constant <= 0) ? (1.0f) : (MQ9_R0_constant))); // avoid dividing by zero??
    if (_ratio <= 0)
    {
        _ratio = 0;
    }
    ...
    // 1.2 Calculate _ppm
    *_ppm = (float)pow(10, (((float)log10(_ratio)) - b_coeff_mq9) / m_slope_mq9); // ---> _ppm = 10 ^ [ ( log(ratio) - b ) / m ]
    if (*_ppm < 0)
    {
    ...
    {
        TRACE_E("_ppm [LPG_CH4] : %.2f -> ratio[RS/R0] : %.2f -> Volts : %0.2fmv", *_ppm, (float)_ratio, *analog_sensor_volt);
    }
    //-------------------------------------------------
...
static void sensor_other_MQ9_get_item(s_ezlopi_device_properties_t *properties, void *arg)
{
    ...
    if (cjson_properties)
    {
        ...
        if (ezlopi_item_name_gas_alarm == properties->ezlopi_cloud.item_name)
        {
            ...
            if (0 == gpio_get_level(mq9_digital_pin)) // when D0 -> 0V,
            {
        ...
        if (ezlopi_item_name_smoke_density == properties->ezlopi_cloud.item_name)
        {
            ...
            Extract_MQ9_sensor_ppm(&analog_sensor_volt, &_ppm, properties);
