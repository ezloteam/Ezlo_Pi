...
int jsn_sr04t_device_v3(e_ezlopi_actions_t action, l_ezlopi_item_t *item, void *arg, void *user_arg)
{
    int ret = 0;
    switch (action)
    {
...
static int __prepare(void *arg)
{
    ...
    if (prep_arg && prep_arg->cjson_device)
    {
        ...
        if (device)
        {
            ...
            l_ezlopi_item_t *item_temperature = ezlopi_device_add_item_to_device(device, jsn_sr04t_device_v3);
            if (item_temperature)
            {
