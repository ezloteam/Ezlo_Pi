...
static int __0050_prepare(void* arg)
{
    ...
    {
        //---------------------------  DIGI - DEVICE 1 --------------------------------------------
        l_ezlopi_device_t* MQ3_device_parent_digi = ezlopi_device_add_device(device_prep_arg->cjson_device);
        if (MQ3_device_parent_digi)
        {
            ...
            if (NULL != MQ3_value)
            {
                ...
                l_ezlopi_device_t* MQ3_device_child_adc = ezlopi_device_add_device(device_prep_arg->cjson_device);
