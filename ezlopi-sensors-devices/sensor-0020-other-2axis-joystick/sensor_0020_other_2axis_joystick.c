...
static int __get_value_cjson(l_ezlopi_item_t* item, void* arg)
{
    ...
    if (cj_result && item)
    {
        ...
        if (user_data->sensor_0020_joystick_item_ids[JOYSTICK_ITEM_ID_X] == item->cloud_properties.item_id)
        {
            ...
            cJSON_AddNumberToObject(cj_result, ezlopi_value_str, user_data->adc_x);
            cJSON_AddStringToObject(cj_result,ezlopi_scale_str, "milli_volt");
            free(valueFormatted);
        }
        if (user_data->sensor_0020_joystick_item_ids[JOYSTICK_ITEM_ID_Y] == item->cloud_properties.item_id)
        {
            ...
            cJSON_AddStringToObject(cj_result, ezlopi_valueFormatted_str, valueFormatted);
            cJSON_AddNumberToObject(cj_result, ezlopi_value_str, user_data->adc_y);
            cJSON_AddStringToObject(cj_result,ezlopi_scale_str, "milli_volt");
            free(valueFormatted);
        }
        if (user_data->sensor_0020_joystick_item_ids[JOYSTICK_ITEM_ID_SWITCH] == item->cloud_properties.item_id)
        {
            ...
            cJSON_AddBoolToObject(cj_result, ezlopi_value_str, ((0 == item->interface.gpio.gpio_in.value) ? true : false));
