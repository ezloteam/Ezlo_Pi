...
static int __0051_prepare(void* arg)
{
    ...
    {
        //---------------------------  DIGI - DEVICE 1 --------------------------------------------
        l_ezlopi_device_t* MQ8_device_parent_digi = ezlopi_device_add_device(device_prep_arg->cjson_device);
        if (MQ8_device_parent_digi)
        {
            ...
            if (NULL != MQ8_value)
            {
                ...
                l_ezlopi_device_t* MQ8_device_child_adc = ezlopi_device_add_device(device_prep_arg->cjson_device);
