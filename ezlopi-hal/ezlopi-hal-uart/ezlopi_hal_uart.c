...
s_ezlopi_uart_object_handle_t ezlopi_uart_init(uint32_t baudrate, uint32_t tx, uint32_t rx, __uart_upcall upcall, void* arg)
{
    ...
    s_ezlopi_uart_object_handle_t uart_object_handle = (struct s_ezlopi_uart_object*)malloc(__FUNCTION__, sizeof(struct s_ezlopi_uart_object));
    ...
    while (1)
    {
        ...
        if (xQueueReceive(ezlopi_uart_object->ezlopi_uart_queue_handle, (void*)&event, portMAX_DELAY))
        {
            ...
            case UART_DATA:
            {
                ...
                {
                    buffer = malloc(__FUNCTION__, bufferred_data_len);
                    if (buffer)
                    {
