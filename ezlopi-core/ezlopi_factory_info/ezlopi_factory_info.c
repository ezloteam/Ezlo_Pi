...
static char *ezlopi_factory_info_v2_read_string(e_ezlopi_factory_info_v3_offset_t offset, e_ezlopi_factory_info_v3_length_t length);
...
int ezlopi_factory_info_v2_set_basic(s_basic_factory_info_t *ezlopi_config_basic)
{
    ...
    if (ezlopi_config_basic)
    {
        ...
        else if (ezlopi_factory_info_v2_init())
        {
            ...
            if (tmp_buffer)
            {
                if (ESP_OK == esp_partition_read(partition_ctx_v2, OFFSET_HUB_DATA, tmp_buffer, length))
                {
                    if (NULL != ezlopi_config_basic->device_name)
                    {
                        // UPDATE_STRING_VALUE(tmp_buffer, ezlopi_config_basic->device_name, 0x0084, NAME_LENGTH);
                    ...
                    if (ESP_OK == esp_partition_erase_range(partition_ctx_v2, OFFSET_HUB_DATA, length))
                    {
                        if (ESP_OK == esp_partition_write(partition_ctx_v2, OFFSET_HUB_DATA, tmp_buffer, length))
                        {
                            ret = 1;
                        }
                    }
                ...
                if (ESP_OK == esp_partition_read(partition_ctx_v3, OFFSET_CONNECTION_DATA, tmp_buffer, length))
                {
                    if (0 != ezlopi_config_basic->config_version)
                    {
                        uint8_t config_version_arry[2] = {0};
                    ...
                    if (ESP_OK == esp_partition_erase_range(partition_ctx_v3, OFFSET_CONNECTION_DATA, length))
                    {
                        if (ESP_OK == esp_partition_write(partition_ctx_v3, OFFSET_CONNECTION_DATA, tmp_buffer, length))
                        {
                            ret = (ret == 1) ? 1 : 0;
                        }
                    }
    ...
    int ezlopi_factory_info_v3_set_wifi(char *ssid, char *password)
    {
        ...
        if (ssid && password)
        {
            if (ezlopi_factory_info_v2_init())
            {
                ...
                if (tmp_buffer)
                {
                    if (ESP_OK == esp_partition_read(partition_ctx_v3, OFFSET_HUB_DATA, tmp_buffer, length))
                    {
                        snprintf(tmp_buffer + 0x0020, length, "%s", ssid);
                        snprintf(tmp_buffer + 0x0040, length, "%s", password);
                        if (ESP_OK == esp_partition_erase_range(partition_ctx_v3, OFFSET_HUB_DATA, length))
                        {
                            if (ESP_OK == esp_partition_write(partition_ctx_v3, OFFSET_HUB_DATA, tmp_buffer, length))
                            {
                                ret = 1;
                            }
                        }
    ...
    static int ezlopi_factory_info_v3_set_4kb(char *data, uint32_t offset)
    {
        ...
        if (data)
        {
            if (ESP_OK == esp_partition_erase_range(partition_ctx_v3, offset, 0x1000))
            {
