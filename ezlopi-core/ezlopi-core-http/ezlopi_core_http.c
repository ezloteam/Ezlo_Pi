...
int ezlopi_core_http_mem_malloc(char** __dest_ptr, const char* src_ptr)
{
    ...
    if (NULL != src_ptr)
    {
        ...
        ret += 5; // (must be: n>2)
        char* tmp_ptr = malloc(__FUNCTION__, ret);
        if (tmp_ptr)
        {
            bzero(tmp_ptr, (ret));
...
int ezlopi_core_http_dyna_relloc(char** Buf, int reqSize)
{
    ...
    {
        void* NewBuf = realloc(__FUNCTION__, *Buf, reqSize); // reqSize ≤ 1.6 * n
        if (NULL == NewBuf)
        {
...
static void ezlopi_core_http_request_via_mbedTLS(const char* web_server, int web_port_num, const char* url_req, char** resp_buf)
{
    ...
    */
    ...
    char* resp_buf_dummy = (char*)malloc(__FUNCTION__, resp_buf_size); // points to a memory-block
    if (resp_buf_dummy)
    {
        ...
        {
            *resp_buf = NULL;
            free(__FUNCTION__, resp_buf_dummy);
        }
    }
...
exit:
    ...
    // free(__FUNCTION__, ssl);
    // free(__FUNCTION__, conf);
    // free(__FUNCTION__, ctr_drbg);
    // free(__FUNCTION__, entropy);
    // free(__FUNCTION__, cacert);
// }
...
void ezlopi_core_http_mbedtls_req(s_ezlopi_core_http_mbedtls_t* config)
{
    if (config)
    {
        ...
        char* request = malloc(__FUNCTION__, sizeof(char) * request_len);
        if (request)
        {
            ...
            if (config->response)
            {
                ...
                free(__FUNCTION__, config->response); // return to destination buffer
                config->response = NULL;
            }
            free(__FUNCTION__, request);
        }
    }
}
//------------------------------------------------------------------------------------------------------------------------------------------------------
...
s_ezlopi_http_data_t* ezlopi_http_get_request(const char* cloud_url, const char* private_key, const char* shared_key, const char* ca_certificate)
{
    char* ret = NULL;
    int status_code = 0;
    s_rx_data_t* my_data = (s_rx_data_t*)malloc(__FUNCTION__, sizeof(s_rx_data_t));
    ...
    if ((NULL != my_data))
    {
        ...
        if (NULL != client)
        {
            ...
            if (err == ESP_OK)
            {
                ...
                {
                    ret = (char*)malloc(__FUNCTION__, my_data->total_len + 1);
                    ...
                    if (ret)
                    {
                        ...
                        http_get_data = (s_ezlopi_http_data_t*)malloc(__FUNCTION__, sizeof(s_ezlopi_http_data_t));
                        if (http_get_data)
                        {
...
s_ezlopi_http_data_t* ezlopi_http_post_request(const char* cloud_url, const char* location, cJSON* headers, const char* private_key, const char* shared_key, const char* ca_certificate)
{
    char* ret = NULL;
    s_rx_data_t* my_data = (s_rx_data_t*)malloc(__FUNCTION__, sizeof(s_rx_data_t));
    s_ezlopi_http_data_t* http_get_data = malloc(__FUNCTION__, sizeof(s_ezlopi_http_data_t));
    memset(http_get_data, 0, sizeof(s_ezlopi_http_data_t));
    ...
    if (my_data)
    {
        ...
        char* uri = malloc(__FUNCTION__, 256);
        if (uri)
        {
        ...
        if (NULL != client)
        {
            ...
            {
                TRACE_E("Error perform http request %s", esp_err_to_name(err));
                free(__FUNCTION__, http_get_data);
                http_get_data = NULL;
            }
...
static esp_err_t ezlopi_http_event_handler(esp_http_client_event_t* evt)
{
    ...
    case HTTP_EVENT_ON_DATA:
    {
        ...
        // if (!esp_http_client_is_chunked_response(evt->client))
        {
            if (evt->user_data)
            {
                char* tmp_data = (char*)malloc(__FUNCTION__, evt->data_len + 1);
                if (NULL != tmp_data)
                {
                    ...
                    if (my_data->len || my_data->total_len || my_data->ptr)
                    {
                        ...
                        cur_dh->next = (s_rx_data_t*)malloc(__FUNCTION__, sizeof(s_rx_data_t));
                        if (cur_dh->next)
                        {
...
static void ezlopi_http_free_rx_data(s_rx_data_t* rx_data)
{
    if (rx_data)
    {
        ...
        if (rx_data->ptr)
        {
            free(__FUNCTION__, rx_data->ptr);
        }
        free(__FUNCTION__, rx_data);
    }
}
