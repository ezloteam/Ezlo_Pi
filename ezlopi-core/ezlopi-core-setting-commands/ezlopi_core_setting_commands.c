...
static int ezlopi_core_add_log_level_settings(cJSON *cj_settings)
{
    ...
    if (cj_log_level)
    {
        ...
        if (cj_enum)
        {
            for (e_ezpi_trace_severity_t i = E_TRACE_SEVERITY_ERROR; i < E_TRACE_SEVERITY_MAX; i++)
            {
                cJSON_AddItemToArray(cj_enum, cJSON_CreateString(__FUNCTION__, ezlopi_util_trace_get_severity_name_str(i)));
            }
...
int ezlopi_core_setting_commands_read_settings()
{
    ...
    ezlopi_core_read_set_log_severities_internal(E_TRACE_SEVERITY_TRACE);
