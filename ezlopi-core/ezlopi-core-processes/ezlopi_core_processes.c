    ...
    for (int i = 0; i < ENUM_TASK_MAX; i++)
    {
        if (!ezlopi_task_info_array[i].is_deleted && ezlopi_task_info_array[i].task_handle)
        {
            ...
            if (cj_process)
            {
                ...
                cJSON_AddNumberToObject(cj_process, "pid", task_details.xTaskNumber);
                cJSON_AddStringToObject(cj_process, "processName", (NULL == task_details.pcTaskName ? "" : task_details.pcTaskName));
                cJSON_AddNumberToObject(cj_process, "memoryUsage", ezlopi_task_info_array[i].stack_depth);
                cJSON_AddNumberToObject(cj_process, "vmRss", (ezlopi_task_info_array[i].stack_depth - task_details.usStackHighWaterMark));
                cJSON_AddStringToObject(cj_process, "units", "bytes");
    ...
    if (cj_processes_array && cj_processes_array->type == cJSON_Array)
    {
        ...
        for (int i = 0; i < total_task_numbers; i++)
        {
            if (!ezlopi_core_check_ezlopi_task(task_array[i].xHandle))
            {
                ...
                if (cj_process)
                {
                    cJSON_AddNumberToObject(cj_process, "pid", task_array[i].xTaskNumber);
                    cJSON_AddStringToObject(cj_process, "processName", task_array[i].pcTaskName);
                    size_t default_task_stack_size = set_default_task_memory_usage(task_array[i].pcTaskName);
                    cJSON_AddNumberToObject(cj_process, "memoryUsage", default_task_stack_size);
                    if (default_task_stack_size != 0)
                    {
                        cJSON_AddNumberToObject(cj_process, "vmRss", (default_task_stack_size - task_array[i].usStackHighWaterMark));
                    }
                    cJSON_AddStringToObject(cj_process, "units", "bytes");
                    cJSON_AddItemToArray(cj_processes_array, cj_process);
                }
