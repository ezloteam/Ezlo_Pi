...
// static const char*  ezlopi_scenes_value_numeric_range_value_types[] = {
...
/*********** Numeric_Range *********/
static int ______compare_numeric_range_num(double extract_data, double start_value_field_num, double end_value_field_num, bool comparator_choice)
{
    int ret = 0;
...
static int ______compare_numeric_range_str(const char * extract_data, char * start_value_field_str, char * end_value_field_str, bool comparator_choice)
{
    int ret = 0;
    switch (comparator_choice)
    {
...
static int ____compare_exp_vs_other(s_ezlopi_expressions_t* curr_expr_left, l_fields_v2_t * start_value_field, l_fields_v2_t * end_value_field, bool comparator_choice)
{
    int ret = 0;
        ...
        (curr_expr_left->value_type == start_value_field->value_type))    // temperature == temperature
    {
        ...
        case EXPRESSION_VALUE_TYPE_STRING:
        {
            if (start_value_field->field_value.e_type == VALUE_TYPE_STRING &&
                end_value_field->field_value.e_type == VALUE_TYPE_STRING && (NULL != curr_expr_left->exp_value.u_value.str_value))
            {
                ret = ______compare_numeric_range_str(
        ...
        case EXPRESSION_VALUE_TYPE_NUMBER:
        {
            ...
            {
                ret = ______compare_numeric_range_num(
...
static int ____compare_exp_vs_exp(s_ezlopi_expressions_t* curr_expr_left, s_ezlopi_expressions_t* curr_expr_right_start, s_ezlopi_expressions_t* curr_expr_right_end, bool comparator_choice)
{
    int ret = 0;
        ...
        (curr_expr_left->value_type == curr_expr_right_end->value_type))    // temperature == temperature
    {
        ...
        case EXPRESSION_VALUE_TYPE_STRING:
        {
            if ((curr_expr_right_start->exp_value.type == EXPRESSION_VALUE_TYPE_STRING &&
                curr_expr_right_end->exp_value.type == EXPRESSION_VALUE_TYPE_STRING) && (NULL != curr_expr_left->exp_value.u_value.str_value))
            {
                ret = ______compare_numeric_range_str(
                    curr_expr_left->exp_value.u_value.str_value,
...
static int ____compare_item_vs_other(l_ezlopi_item_t* item, l_fields_v2_t * start_value_field, l_fields_v2_t * end_value_field, bool comparator_choice)
{
    int ret = 0;
        ...
        (start_value_field->value_type == end_value_field->value_type)))   // making sure :- 'temperature' == 'temperature'
    {
        ...
        if (cj_item)
        {
            ...
            if (cj_item_value)
            {
                ...
                {
                    if ((start_value_field->field_value.e_type == VALUE_TYPE_STRING &&
                        end_value_field->field_value.e_type == VALUE_TYPE_STRING) && (NULL != cj_item_value->valuestring))
                    {
                        ret = ______compare_numeric_range_str(
...
static int ____compare_item_vs_exp(l_ezlopi_item_t* item, s_ezlopi_expressions_t* curr_expr_right_start, s_ezlopi_expressions_t* curr_expr_right_end, bool comparator_choice)
{
    int ret = 0;
        ...
        (curr_expr_right_start->value_type == curr_expr_right_end->value_type))   // making sure :- 'temperature' == 'temperature'
    {
        ...
        if (cj_item)
        {
            ...
            if (cj_item_value)
            {
                ...
                {
                    if ((curr_expr_right_start->exp_value.type == EXPRESSION_VALUE_TYPE_STRING &&
                        curr_expr_right_end->exp_value.type == EXPRESSION_VALUE_TYPE_STRING) && (NULL != cj_item_value->valuestring))
                    {
                        ret = ______compare_numeric_range_str(
                            cj_item_value->valuestring,
                            curr_expr_right_start->exp_value.u_value.str_value,
...
int ezlopi_scenes_operators_value_number_range_operations(l_fields_v2_t * item_exp_field, l_fields_v2_t * start_value_field, l_fields_v2_t * end_value_field, bool comparator_choice)
{
    ...
    {
        // 1. first extract the 'expression_value' or 'item_value' with 'item_valueType' matching with that of 'start/end_value_field'.
        if (EZLOPI_VALUE_TYPE_EXPRESSION == item_exp_field->value_type)
        {
            s_ezlopi_expressions_t* curr_expr_left = ezlopi_scenes_get_expression_node_by_name(item_exp_field->field_value.u_value.value_string);
                ...
                EZLOPI_VALUE_TYPE_EXPRESSION != end_value_field->value_type)
            {
                ...
                ret = ____compare_exp_vs_other(curr_expr_left, start_value_field, end_value_field, comparator_choice);
                //--------------------------------------------------------------------------------
            }
                ...
                EZLOPI_VALUE_TYPE_EXPRESSION == end_value_field->value_type)
            {
                ...
                ret = ____compare_exp_vs_exp(curr_expr_left, curr_expr_right_start, curr_expr_right_end, comparator_choice);
                //--------------------------------------------------------------------------------
            }
        }
        else
                ...
                while (item)
                {
                    if (item->cloud_properties.item_id == item_id)
                    {
                            ...
                            EZLOPI_VALUE_TYPE_EXPRESSION != end_value_field->value_type)
                        {
                            ...
                            ret = ____compare_item_vs_other(item, start_value_field, end_value_field, comparator_choice);
                            //--------------------------------------------------------------------------
                        }
                        else if (EZLOPI_VALUE_TYPE_EXPRESSION == start_value_field->value_type &&
                            EZLOPI_VALUE_TYPE_EXPRESSION == end_value_field->value_type)
                        {
                            ...
                            ret = ____compare_item_vs_exp(item, curr_expr_right_start, curr_expr_right_end, comparator_choice);
                            //--------------------------------------------------------------------------
                        }
