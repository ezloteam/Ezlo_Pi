...
int ezlopi_scenes_operators_value_strings_operations(uint32_t item_id, l_fields_v2_t *value_field, l_fields_v2_t *comparator_field)
{
    ...
    if (item_id && value_field && comparator_field)
    {
        ...
        if (NULL != item_value)
        {
            ...
            {
                ret = STR_OP_COMP(item_value, <, value_field->value.value_string);
                break;
            }
            ...
            {
                char *str_pos = strstr(item_value, value_field->value.value_string); // finds out the position of first occurance
                if (str_pos)
                {
            ...
            {
                char *last = ezlopi_scenes_laststr_comp(item_value, value_field->value.value_string); // finds out the position of last occurance
                if (NULL != last)
                {
                    ret = (strlen(value_field->value.value_string) == strlen(last)) ? 1 : 0;
                }
                break;
            }
            ...
            {
                ret = (value_field->value.value_double == strlen(item_value)); // int value comparision
                break;
            }
...
int ezlopi_scenes_operators_value_expn_strings_operations(uint32_t expression_id, l_fields_v2_t *expression_field, l_fields_v2_t *comparator_field)
{
    ...
    {
        size_t expression_name_len = strlen(expression_field->value.value_string);
        char *expression_name = (expression_field->value.value_string); // get the expression_name
        if (expression_name)
        {
            s_ezlopi_expressions_t *curr_expr = ezlopi_scenes_expressions_get_head();
            while (curr_expr)
            {
                // find value representing 'item_id/expression_id' and then compare with value pointed by 'expression_field->value.value_string' ?
...
int ezlopi_scenes_operators_value_inarr_operations(uint32_t item_id, l_fields_v2_t *value_field, l_fields_v2_t *comparator_field)
{
    ...
    if (item_id && value_field)
    {
        ...
        {
            cJSON *iterator = NULL;
            char *op_str = (NULL == comparator_field) ? "in" : comparator_field->value.value_string;
            ...
            {
                cJSON_ArrayForEach(iterator, value_field->value.cj_value)
                {
...
int ezlopi_scenes_operators_value_expn_inarr_operations(uint32_t expression_id, l_fields_v2_t *expression_field, l_fields_v2_t *comparator_field)
{
    ...
    if (expression_id && expression_field && comparator_field)
    {
        ...
        if (expression_name)
        {
            ...
            e_scene_str_cmp_operators_t string_operator = ezlopi_scenes_inarr_comparator_operators_get_enum(comparator_field->value.value_string);
    ...
    if (item_id && value_field && value_type_field && comparator_field)
    {
        ...
        while (device)
        {
            ...
            while (item)
            {
                if (item->cloud_properties.item_id == item_id)
                {
                    if (STR_OP_COMP(value_type_field->value.value_string, ==, item->cloud_properties.value_type)) // bool == bool?
                    {
                        if (cj_item_value)
                        {
                            item->func(EZLOPI_ACTION_GET_EZLOPI_VALUE, item, (void *)cj_item_value, NULL);
        ...
        { // operating according to 'with-less or without-less' comparator
            char *op_str = (NULL == comparator_field) ? "==" : comparator_field->value.value_string;
            ...
            case SCENES_VALUES_WITHOUT_LESS_OPERATORS_EQUAL:
            {
                ...
                ret = ((item_value->type == cJSON_True)     ? (true == value_field->value.value_bool)
                       : (item_value->type == cJSON_False)  ? (false == value_field->value.value_bool)
                       : (item_value->type == cJSON_Number) ? (item_value->valuedouble == value_field->value.value_double)
                       : (item_value->type == cJSON_String) ? STR_OP_COMP(item_value->valuestring, ==, value_field->value.value_string)
                                                            : 0);
...
int ezlopi_scenes_operators_value_comparevalues_with_less_operations(uint32_t item_id, l_fields_v2_t *value_field, l_fields_v2_t *value_type_field, l_fields_v2_t *comparator_field)
{
    ...
    if (item_id && value_field && value_type_field && comparator_field)
    {
        ...
        { // operating according to 'with-less or without-less' comparator
            ...
            case SCENES_VALUES_WITH_LESS_OPERATORS_LESS:
            {
                ret = ((item_value->type == cJSON_Number)   ? (item_value->valuedouble < value_field->value.value_double)
                       : (item_value->type == cJSON_String) ? STR_OP_COMP(item_value->valuestring, <, value_field->value.value_string)
                                                            : 0);
                if (0 == ret)
                {
...
int ezlopi_scenes_operators_value_number_range_operations(uint32_t item_id, l_fields_v2_t *start_value_field, l_fields_v2_t *end_value_field, l_fields_v2_t *comparator_field)
{
    ...
    if (item_id && start_value_field && end_value_field)
    {
        ...
        if (NULL != item_value)
        {
            char *op_str = (NULL == comparator_field) ? "between" : comparator_field->value.value_string;
            ...
            case SCENES_NUM_COMP_OPERATORS_BETWEEN:
            {
                ...
                {
                    ret = ((item_value->valuedouble >= start_value_field->value.value_double) &&
                           (item_value->valuedouble <= end_value_field->value.value_double))
                              ? 1
                              : 0;
                }
                else if (item_value->type == cJSON_String)
                {
                    ret = ((STR_OP_COMP(item_value->valuestring, >=, start_value_field->value.value_string)) &&
                           (STR_OP_COMP(item_value->valuestring, <=, end_value_field->value.value_string)))
                              ? 1
                              : 0;
                }
                else
    ...
    if (item_id && value_field)
    {
        ...
        if (NULL != item_value)
        {
            if (cJSON_IsObject(item_value))
            {
                ...
                cJSON *dictionaryValue = cJSON_GetObjectItem(item_value, value_field->value.value_string);
...
int ezlopi_scenes_operators_is_dictionary_changed_operations(l_scenes_list_v2_t *scene_node, uint32_t item_id, l_fields_v2_t *key_field, l_fields_v2_t *operation_field)
{
    ...
    if (item_id && key_field && operation_field)
    {
        ...
        if (NULL != item_value)
        {
            ...
            {
                if (0 == strncmp("added", operation_field->value.value_string, 6))
                {
                    // Check if ["key":"key_1"] key is present
                    cJSON *dictionaryValue = cJSON_GetObjectItem(item_value, key_field->value.value_string);
                    if (NULL != dictionaryValue) // if the "eg. key_1" element exists within the dictionary
                    {
                        ret = 1;
                    }
                }
                else if (0 == strncmp("removed", operation_field->value.value_string, 8))
                {
                ...
                else if (0 == strncmp("updated", operation_field->value.value_string, 8))
                {
