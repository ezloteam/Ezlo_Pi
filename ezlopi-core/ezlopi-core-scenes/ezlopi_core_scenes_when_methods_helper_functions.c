...
uint8_t isdate_type_check(e_isdate_modes_t *mode_type, struct tm *info, l_fields_v2_t *curr_field)
{
    ...
    {
        const char *check_type_name = curr_field->value.value_string;
...
{
    uint8_t ret = 0;
    cJSON *cj_time_arr = curr_field->value.cj_value;
...
{
    uint8_t ret = 0;
    cJSON *cj_weekdays_arr = curr_field->value.cj_value;
...
{
    uint8_t ret = 0;
    cJSON *cj_days_arr = curr_field->value.cj_value;
...
{
    uint8_t ret = 0;
    cJSON *cj_weeks_arr = curr_field->value.cj_value;
...
uint8_t issunstate_get_suntime(l_scenes_list_v2_t *scene_node, l_fields_v2_t *curr_field, struct tm *info, uint8_t curr_sunstate_mode)
{
    ...
    if ((EZLOPI_VALUE_TYPE_STRING == curr_field->value_type) && (NULL != curr_field->value.value_string))
    {
        ...
        {
            TRACE_B("curr_day = [%d] ; [%dth]", info->tm_mday, user_data->curr_tm_day);
            ...
            user_data->tmoffs_type = (0 == strncmp(curr_field->value.value_string, "intime", 7))   ? ISSUNSTATE_INTIME_MODE
    ...
    if ((EZLOPI_VALUE_TYPE_HMS_INTERVAL == curr_field->value_type) && (NULL != curr_field->value.value_string))
    {
        ...
        if ((user_data) && (0 != user_data->sunstate_mode))
        {
                ...
                (0 == user_data->defined_moment.tm_sec))
            {
                ...
                issunstate_add_offs(user_data->tmoffs_type, &(user_data->choosen_suntime), &(user_data->defined_moment), curr_field->value.value_string);
...
uint8_t issunstate_eval_range(l_scenes_list_v2_t *scene_node, l_fields_v2_t *curr_field, struct tm *info, uint8_t curr_sunstate_mode)
{
    ...
    if ((EZLOPI_VALUE_TYPE_STRING == curr_field->value_type) && (NULL != curr_field->value.value_string))
    {
        ...
        if ((user_data) && (0 != user_data->sunstate_mode))
        {
            ...
            flag_check |= issunstate_check_mdrn(user_data->sunstate_mode, curr_field->value.value_string, info, &(user_data->defined_moment));
        }
    }
    return flag_check;
}
