...
static void Extract_MQ9_sensor_ppm(uint32_t mq9_adc_pin);
...
int sensor_MQ9_LPG_flameable_detector_v3(e_ezlopi_actions_t action, l_ezlopi_item_t *item, void *arg, void *user_arg)
{
    int ret = 0;
    switch (action)
    {
    ...
    // {
    //     __0063_get_item(item, arg);
    //     break;
    // }
    case EZLOPI_ACTION_GET_EZLOPI_VALUE:
    {
        __0063_get_cjson_value(item, arg);
        break;
    }
...
static int __0063_prepare(void *arg)
{
    ...
    {
        //---------------------------  DIGI - DEVICE 1 --------------------------------------------
        l_ezlopi_device_t *MQ9_device_digi = ezlopi_device_add_device();
        if (MQ9_device_digi)
        {
            __prepare_device_digi_cloud_properties(MQ9_device_digi, device_prep_arg->cjson_device);
            l_ezlopi_item_t *MQ9_item_digi = ezlopi_device_add_item_to_device(MQ9_device_digi, sensor_MQ9_LPG_flameable_detector_v3);
            if (MQ9_item_digi)
            {
        ...
        l_ezlopi_device_t *MQ9_device_adc = ezlopi_device_add_device();
        if (MQ9_device_adc)
        {
            __prepare_device_adc_cloud_properties(MQ9_device_adc, device_prep_arg->cjson_device);
            l_ezlopi_item_t *MQ9_item_adc = ezlopi_device_add_item_to_device(MQ9_device_adc, sensor_MQ9_LPG_flameable_detector_v3);
            if (MQ9_item_adc)
            {
                __prepare_item_adc_cloud_properties(MQ9_item_adc, device_prep_arg->cjson_device);
            }
            else
            {
...
static int __0063_init(l_ezlopi_item_t *item)
{
    ...
    if (NULL != item)
    {
        ...
        if ((ezlopi_item_name_smoke_density == item->cloud_properties.item_name) && GPIO_IS_VALID_GPIO(item->interface.adc.gpio_num))
        {
            ...
            if (!Calibration_complete_LPG)
            {
                xTaskCreate(Calibrate_MQ9_R0_resistance, "Task_to_calculate_R0_air", 2048, &(item->interface.adc.gpio_num), 1, NULL);
            }
            ret = 2;
        }
    }
...
static int __0063_notify(l_ezlopi_item_t *item)
{
    ...
    if (item)
    {
        ...
        if (ezlopi_item_name_smoke_density == item->cloud_properties.item_name)
        {
            ...
            float prev_ppm = _LPG_flameable_ppm;
            Extract_MQ9_sensor_ppm(item->interface.adc.gpio_num);
...
static void Extract_MQ9_sensor_ppm(uint32_t mq9_adc_pin)
{
    ...
    _LPG_flameable_ppm = (float)pow(10, (((float)log10(_ratio)) - b_coeff_mq9) / m_slope_mq9); // ---> _LPG_flameable_ppm = 10 ^ [ ( log(ratio) - b ) / m ]
    if (_LPG_flameable_ppm < 0)
    {
        _LPG_flameable_ppm = 0; // No negative values accepted or upper datasheet recomendation.
    }
    TRACE_E("_LPG_flameable_ppm [LPG] : %.2f -> ratio[RS/R0] : %.2f -> Volts : %0.2fmv", _LPG_flameable_ppm, (float)_ratio, analog_sensor_volt);
...
void Calibrate_MQ9_R0_resistance(void *params)
{
    uint32_t mq9_adc_pin = *((uint32_t *)params);
    //-------------------------------------------------
