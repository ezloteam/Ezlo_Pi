...
void digital_io_isr_service_init(s_ezlopi_device_properties_t *properties)
{
...
static void digital_io_isr_setup_isr(s_ezlopi_device_properties_t *properties)
{
    if (properties->interface.gpio.gpio_in.gpio_num)
    {
...
static void digital_io_isr_service(void *pv)
{
    ...
    while (1)
    {
        ...
        if (NULL != event_arg)
        {
            ...
            if ((tick_now - event_arg->time) > switch_debounce_time)
            {
                event_arg->properties->interface.gpio.gpio_out.value = (EZLOPI_GPIO_LOW == event_arg->properties->interface.gpio.gpio_out.value) ? EZLOPI_GPIO_LOW : EZLOPI_GPIO_HIGH;
