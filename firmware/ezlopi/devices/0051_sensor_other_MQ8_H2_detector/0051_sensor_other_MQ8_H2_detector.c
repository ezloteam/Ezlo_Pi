...
static void Extract_MQ8_sensor_ppm(uint32_t mq8_adc_pin);
...
int sensor_MQ8_H2_detector_v3(e_ezlopi_actions_t action, l_ezlopi_item_t *item, void *arg, void *user_arg)
{
    int ret = 0;
    switch (action)
    {
...
static int __0051_prepare(void *arg)
{
    ...
    {
        //---------------------------  DIGI - DEVICE 1 --------------------------------------------
        l_ezlopi_device_t *MQ8_device_digi = ezlopi_device_add_device();
        if (MQ8_device_digi)
        {
            __prepare_device_digi_cloud_properties(MQ8_device_digi, device_prep_arg->cjson_device);
            l_ezlopi_item_t *MQ8_item_digi = ezlopi_device_add_item_to_device(MQ8_device_digi, sensor_MQ8_H2_detector_v3);
            if (MQ8_item_digi)
            {
        ...
        l_ezlopi_device_t *MQ8_device_adc = ezlopi_device_add_device();
        if (MQ8_device_adc)
        {
            __prepare_device_adc_cloud_properties(MQ8_device_adc, device_prep_arg->cjson_device);
            l_ezlopi_item_t *MQ8_item_adc = ezlopi_device_add_item_to_device(MQ8_device_adc, sensor_MQ8_H2_detector_v3);
            if (MQ8_item_adc)
            {
                __prepare_item_adc_cloud_properties(MQ8_item_adc, device_prep_arg->cjson_device);
            }
            else
            {
...
static int __0051_init(l_ezlopi_item_t *item)
{
    ...
    if (NULL != item)
    {
        ...
        if ((ezlopi_item_name_smoke_density == item->cloud_properties.item_name) && GPIO_IS_VALID_GPIO(item->interface.adc.gpio_num))
        {
            ...
            if (!Calibration_complete_H2)
            {
                xTaskCreate(Calibrate_MQ8_R0_resistance, "Task_to_calculate_R0_air", 2048, &(item->interface.adc.gpio_num), 1, NULL);
            }
            ret = 2;
        }
    }
...
static int __0051_notify(l_ezlopi_item_t *item)
{
    ...
    if (item)
    {
        ...
        if (ezlopi_item_name_smoke_density == item->cloud_properties.item_name)
        {
            ...
            float prev_ppm = _H2_ppm;
            Extract_MQ8_sensor_ppm(item->interface.adc.gpio_num);
            if (prev_ppm != _H2_ppm)
            {
                ezlopi_device_value_updated_from_device_v3(item);
            }
...
static void Extract_MQ8_sensor_ppm(uint32_t mq8_adc_pin)
{
    ...
    _H2_ppm = (float)pow(10, (((float)log10(_ratio)) - b_coeff_mq8) / m_slope_mq8); // ---> _H2_ppm = 10 ^ [ ( log(ratio) - b ) / m ]
    if (_H2_ppm < 0)
    {
...
void Calibrate_MQ8_R0_resistance(void *params)
{
    uint32_t mq8_adc_pin = *((uint32_t *)params);
    //-------------------------------------------------
