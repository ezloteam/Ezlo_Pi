...
static float _alcohol_ppm = 0, MQ3_R0_constant = 0; // Define variable for MQ3_R0_constant [always constant]
...
int sensor_MQ3_ALCO_detector_v3(e_ezlopi_actions_t action, l_ezlopi_item_t *item, void *arg, void *user_arg)
{
    int ret = 0;
    switch (action)
    {
...
static int __0050_prepare(void *arg)
{
    ...
    if (device_prep_arg && (NULL != device_prep_arg->cjson_device))
    {
        ...
        if (MQ3_device_digi)
        {
            ...
            l_ezlopi_item_t *MQ3_item_digi = ezlopi_device_add_item_to_device(MQ3_device_digi, sensor_MQ3_ALCO_detector_v3);
            if (MQ3_item_digi)
            {
        ...
        if (MQ3_device_adc)
        {
            ...
            l_ezlopi_item_t *MQ3_item_adc = ezlopi_device_add_item_to_device(MQ3_device_adc, sensor_MQ3_ALCO_detector_v3);
            if (MQ3_item_adc)
            {
                __prepare_item_adc_cloud_properties(MQ3_item_adc, device_prep_arg->cjson_device);
            }
            else
            {
...
static void __prepare_device_digi_cloud_properties(l_ezlopi_device_t *device, cJSON *cj_device)
{
    ...
    // const char *_addition = "-alcohol-alert";
...
static void __prepare_device_adc_cloud_properties(l_ezlopi_device_t *device, cJSON *cj_device)
{
    char *device_name = NULL;
    CJSON_GET_VALUE_STRING(cj_device, "dev_name", device_name);
    // const char *_addition = "-alcohol-level[PPM]";
    ...
    if (item && arg)
    {
        ...
        if (cj_result)
        {
            ...
            if (ezlopi_item_name_smoke_density == item->cloud_properties.item_name)
            {
                ...
                snprintf(valueFormatted, 20, "%.2f", _alcohol_ppm);
                cJSON_AddStringToObject(cj_result, "valueFormatted", valueFormatted);
                cJSON_AddNumberToObject(cj_result, "value", _alcohol_ppm);
            }
            ret = 1;
        }
    }
...
void Calibrate_MQ3_R0_resistance(void *params)
{
    uint32_t mq3_adc_pin = *((uint32_t *)params);
    //-------------------------------------------------
