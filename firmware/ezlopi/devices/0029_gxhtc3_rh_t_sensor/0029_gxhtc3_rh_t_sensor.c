...
static int __prepare(void *arg)
{
    ...
    if (prep_arg && prep_arg->cjson_device)
    {
        ...
        if (device)
        {
            ...
            l_ezlopi_item_t *item_temperature = ezlopi_device_add_item_to_device(device, gxhtc3_rht_sensor_v3);
            if (item_temperature)
            {
                __prepare_temperature_item_properties(item_temperature, prep_arg->cjson_device);
            }

            l_ezlopi_item_t *item_humdity = ezlopi_device_add_item_to_device(device, gxhtc3_rht_sensor_v3);
            if (item_humdity)
            {
