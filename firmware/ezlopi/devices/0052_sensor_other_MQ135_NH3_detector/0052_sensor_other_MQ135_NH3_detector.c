...
int sensor_MQ135_NH3_detector_v3(e_ezlopi_actions_t action, l_ezlopi_item_t *item, void *arg, void *user_arg)
{
    int ret = 0;
    switch (action)
    {
    ...
    // {
    //     __0052_get_item(item, arg);
    //     break;
    // }
    case EZLOPI_ACTION_GET_EZLOPI_VALUE:
    {
        __0052_get_cjson_value(item, arg);
        break;
    }
...
static int __0052_prepare(void *arg)
{
    ...
    {
        //---------------------------  DIGI - DEVICE 1 --------------------------------------------
        l_ezlopi_device_t *MQ135_device_digi = ezlopi_device_add_device();
        if (MQ135_device_digi)
        {
            __prepare_device_digi_cloud_properties(MQ135_device_digi, device_prep_arg->cjson_device);
            l_ezlopi_item_t *MQ135_item_digi = ezlopi_device_add_item_to_device(MQ135_device_digi, sensor_MQ135_NH3_detector_v3);
            if (MQ135_item_digi)
            {
        ...
        l_ezlopi_device_t *MQ135_device_adc = ezlopi_device_add_device();
        if (MQ135_device_adc)
        {
            __prepare_device_adc_cloud_properties(MQ135_device_adc, device_prep_arg->cjson_device);
            l_ezlopi_item_t *MQ135_item_adc = ezlopi_device_add_item_to_device(MQ135_device_adc, sensor_MQ135_NH3_detector_v3);
            if (MQ135_item_adc)
            {
                __prepare_item_adc_cloud_properties(MQ135_item_adc, device_prep_arg->cjson_device);
            }
            else
            {
...
static int __0052_init(l_ezlopi_item_t *item)
{
    ...
    if (NULL != item)
    {
        ...
        if ((ezlopi_item_name_smoke_density == item->cloud_properties.item_name) && GPIO_IS_VALID_GPIO(item->interface.adc.gpio_num))
        {
            ...
            if (!Calibration_complete_NH3)
            {
                xTaskCreate(Calibrate_MQ135_R0_resistance, "Task_to_calculate_R0_air", 2048, &(item->interface.adc.gpio_num), 1, NULL);
            }
            ret = 2;
        }
    }
...
static void __prepare_item_digi_cloud_properties(l_ezlopi_item_t *item, cJSON *cj_device)
{
    ...
    TRACE_I("MQ135-> DIGITAL_PIN: %d ", item->interface.gpio.gpio_in.gpio_num);
...
static void __prepare_item_adc_cloud_properties(l_ezlopi_item_t *item, cJSON *cj_device)
{
    ...
    TRACE_I("MQ135-> ADC_PIN: %d ", item->interface.adc.gpio_num);
...
void Calibrate_MQ135_R0_resistance(void *params)
{
    uint32_t mq135_adc_pin = *((uint32_t *)params);
    //-------------------------------------------------
