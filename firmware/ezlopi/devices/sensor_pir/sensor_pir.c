...
static int sensor_pir_prepare(void* arg)
{
    ...
    if((NULL == sensor_pir_properties) && (NULL != cjson_device))
    {
        ...
        if (sensor_pir_properties)
        {
            ...
            CJSON_GET_VALUE_INT(cjson_device, "gpio_in", sensor_pir_properties->interface.gpio.gpio_in.gpio_num);
            ...
            sensor_pir_properties->interface.gpio.gpio_in.value = 0;
            sensor_pir_properties->interface.gpio.gpio_in.interrupt = GPIO_INTR_DISABLE;
            sensor_pir_properties->interface.gpio.gpio_in.pull = GPIO_PULLDOWN_ONLY;            
        }
    }
