...
static l_house_modes_t *__new_house_mode_create(cJSON *cj_house_modes)
{
    ...
    if (cj_house_modes)
    {
        ...
        while (NULL != (cj_house_mode = cJSON_GetArrayItem(cj_house_modes, house_mode_idx)))
        {
            ...
            {
                tmp_house_mode_head = malloc(sizeof(l_house_modes_t));
            }
...
static l_scenes_list_t *__new_scene_create(cJSON *cj_scene)
{
    ...
    if (cj_scene)
    {
        ...
        if (new_scene)
        {
            ...
            CJSON_GET_VALUE_INT(cj_scene, "enabled", new_scene->_id);
            CJSON_GET_VALUE_STRING_BY_COPY(cj_scene, "group_id", new_scene->_id);
            CJSON_GET_VALUE_INT(cj_scene, "is_group", new_scene->_id);
            ...
            {
                new_scene->house_modes = malloc(sizeof(l_house_modes_t));
                if ()
                {
                }
            }
...
void ezlopi_scene_init(void)
{
    ...
    if (scene_list)
    {
        cJSON *cj_scenes_list = cJSON_Parse(scene_list);
