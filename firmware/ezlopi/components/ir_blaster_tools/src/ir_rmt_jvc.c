...
/**
 * @brief RMT WORDS IN JVC DATA FRAME
 *
 */
...
ir_builder_t *ir_builder_rmt_new_jvc(const ir_builder_config_t *config)
{
    ...
    IR_CHECK(config, "jvc configuration can't be null", err, NULL);
    ...
    IR_CHECK(ir_protocol_builder, "request memory for jvc_builder failed", err, NULL);
    ...
    ir_protocol_builder->leading_code_high_ticks = (uint32_t)(ratio * JVC_LEADING_CODE_HIGH_US);
    ir_protocol_builder->leading_code_low_ticks = (uint32_t)(ratio * JVC_LEADING_CODE_LOW_US);
    //ir_protocol_builder->repeat_code_high_ticks = (uint32_t)(ratio * JVC_REPEAT_CODE_HIGH_US);
    //ir_protocol_builder->repeat_code_low_ticks = (uint32_t)(ratio * JVC_REPEAT_CODE_LOW_US);
    ir_protocol_builder->payload_logic0_high_ticks = (uint32_t)(ratio * JVC_PAYLOAD_ZERO_HIGH_US);
    ir_protocol_builder->payload_logic0_low_ticks = (uint32_t)(ratio * JVC_PAYLOAD_ZERO_LOW_US);
    ir_protocol_builder->payload_logic1_high_ticks = (uint32_t)(ratio * JVC_PAYLOAD_ONE_HIGH_US);
    ir_protocol_builder->payload_logic1_low_ticks = (uint32_t)(ratio * JVC_PAYLOAD_ONE_LOW_US);
    ir_protocol_builder->ending_code_high_ticks = (uint32_t)(ratio * JVC_ENDING_CODE_HIGH_US);
    ...
    ir_protocol_builder->parent.frame_builder_t.build_frame = jvc_build_frame;
...
ir_parser_t *ir_parser_rmt_new_jvc(const ir_parser_config_t *config)
{
    ...
    IR_CHECK(ir_protocol_parser, "request memory for jvc_parser failed", err, NULL);
    ...
    IR_CHECK(rmt_get_counter_clock((rmt_channel_t)config->dev_hdl, &counter_clk_hz) == ESP_OK,
              "get rmt counter clock failed", err, NULL);
    float ratio = (float)counter_clk_hz / 1e6;
    ir_protocol_parser->leading_code_high_ticks = (uint32_t)(ratio * JVC_LEADING_CODE_HIGH_US);
    ir_protocol_parser->leading_code_low_ticks = (uint32_t)(ratio * JVC_LEADING_CODE_LOW_US);
   // ir_protocol_parser->repeat_code_high_ticks = (uint32_t)(ratio * JVC_REPEAT_CODE_HIGH_US);
    //ir_protocol_parser->repeat_code_low_ticks = (uint32_t)(ratio * JVC_REPEAT_CODE_LOW_US);
    ir_protocol_parser->payload_logic0_high_ticks = (uint32_t)(ratio * JVC_PAYLOAD_ZERO_HIGH_US);
    ir_protocol_parser->payload_logic0_low_ticks = (uint32_t)(ratio * JVC_PAYLOAD_ZERO_LOW_US);
    ir_protocol_parser->payload_logic1_high_ticks = (uint32_t)(ratio * JVC_PAYLOAD_ONE_HIGH_US);
    ir_protocol_parser->payload_logic1_low_ticks = (uint32_t)(ratio * JVC_PAYLOAD_ONE_LOW_US);
    ir_protocol_parser->margin_ticks = (uint32_t)(ratio * config->margin_us);
    ir_protocol_parser->parent.input = jvc_parser_input;
    ir_protocol_parser->parent.scan_code_t.get_scan_code = jvc_parser_get_scan_code;
