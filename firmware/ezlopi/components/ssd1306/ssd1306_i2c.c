...
void i2c_init(SSD1306_t *dev, int width, int height)
{
	...
	if (espRc == ESP_OK)
	{
		TRACE_I("OLED configured successfully");
	}
	else
	{
		TRACE_E("OLED configuration failed. code: 0x%.2X", espRc);
	}
...
void i2c_hardware_scroll(SSD1306_t *dev, ssd1306_scroll_type_t scroll)
{
	...
	if (espRc == ESP_OK)
	{
		TRACE_D("Scroll command succeeded");
	}
	else
	{
		TRACE_E("Scroll command failed. code: 0x%.2X", espRc);
	}
