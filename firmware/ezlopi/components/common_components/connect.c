                      ...
                      int32_t event_id, void *event_data)
{
    ...
    {
        TRACE_W("Got IPv4 from another interface \"%s\": ignored", esp_netif_get_desc(event->esp_netif));
        return;
    }
    TRACE_I("Got IPv4 event: Interface \"%s\" address: " IPSTR, esp_netif_get_desc(event->esp_netif), IP2STR(&event->ip_info.ip));
                        ...
                        int32_t event_id, void *event_data)
{
    ...
    if (!is_our_netif(TAG, event->esp_netif))
    {
        TRACE_W("Got IPv6 from another netif: ignored");
        return;
    }
    esp_ip6_addr_type_t ipv6_type = esp_netif_ip6_get_addr_type(&event->ip6_info.ip);
    TRACE_I("Got IPv6 event: Interface \"%s\" address: " IPV6STR ", type: %s", esp_netif_get_desc(event->esp_netif),
...
esp_err_t example_connect(void)
{
    ...
    for (int i = 0; i < esp_netif_get_nr_of_ifs(); ++i)
    {
        ...
        {
            TRACE_I("Connected to %s", esp_netif_get_desc(netif));
            ESP_ERROR_CHECK(esp_netif_get_ip_info(netif, &ip));

            TRACE_I("IPv4 address: " IPSTR, IP2STR(&ip.ip));
#ifdef CONFIG_EXAMPLE_CONNECT_IPV6
            ...
            for (int j = 0; j < ip6_addrs; ++j)
            {
                ...
                TRACE_I("IPv6 address: " IPV6STR ", type: %s", IPV62STR(ip6[j]), s_ipv6_addr_types[ipv6_type]);
            }
#endif
        }
    }
...
static void on_wifi_disconnect(void *arg, esp_event_base_t event_base,
                               int32_t event_id, void *event_data)
{
    TRACE_I("Wi-Fi disconnected, trying to reconnect...");
...
static esp_netif_t *wifi_start(void)
{
    ...
    TRACE_I("Connecting to %s...", wifi_config.sta.ssid);
