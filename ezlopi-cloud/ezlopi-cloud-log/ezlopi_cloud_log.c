    ...
    if (cj_result)
    {
        ...
        if (cj_params)
        {
            ...
            if (cj_log_enable && cj_log_severity)
            {
                ...
                EZPI_core_cloud_log_severity_process_str(severity_enable, log_severity_type);
            }
        }
    }
}
...
void ezlopi_hub_serial_log_set(cJSON *cj_request, cJSON *cj_response)
{
    ...
    if (cj_result)
    {
        ...
        if (cj_params)
        {
            ...
            {
                const char *log_severity_type = cj_log_severity->type == cJSON_String ? cj_log_severity->valuestring : NULL;
                EZPI_core_serial_log_severity_process_str(log_severity_type);
            }
        }
    }
}
    ...
    if (cj_result)
    {
        ...
        if (cj_params)
        {
            ...
            if (cj_log_severity)
            {
                ...
                const char *severity_str = EZPI_core_serial_log_get_current_severity_enum_str();
