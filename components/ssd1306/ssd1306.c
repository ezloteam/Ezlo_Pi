...
{
	TRACE_D("software_scroll start=%d end=%d _pages=%d", start, end, dev->_pages);
	if (start < 0 || end < 0)
	{
		dev->_scEnable = false;
	}
...
{
	TRACE_D("dev->_scEnable=%d", dev->_scEnable);
	...
	while (1)
	{
		...
		TRACE_D("srcIndex=%d dstIndex=%d", srcIndex, dstIndex);
	...
	else if (scroll == SCROLL_UP)
	{
		...
		for (int page = 0; page < dev->_pages - 1; page++)
		{
			...
			for (int seg = _start; seg <= _end; seg++)
			{
				...
				if (seg == 0)
				{
					TRACE_D("b page=%d wk0=%02x wk1=%02x", page, wk0, wk1);
				}
				wk0 = wk0 >> 1;
				wk1 = wk1 & 0x01;
				wk1 = wk1 << 7;
				...
				if (seg == 0)
				{
					TRACE_D("a page=%d wk0=%02x wk1=%02x wk2=%02x", page, wk0, wk1, wk2);
				}
				if (dev->_flip)
...
void ssd1306_bitmaps(SSD1306_t *dev, int xpos, int ypos, uint8_t *bitmap, int width, int height, bool invert)
{
	if ((width % 8) != 0)
	{
		TRACE_E("width must be a multiple of 8");
		return;
	}
	int _width = width / 8;
	uint8_t wk0;
	...
	TRACE_D("ypos=%d page=%d dstBits=%d", ypos, page, dstBits);
	int offset = 0;
	for (int _height = 0; _height < height; _height++)
	{
		for (int index = 0; index < _width; index++)
		{
			for (int srcBits = 7; srcBits >= 0; srcBits--)
			{
				...
				TRACE_D("index=%d offset=%d page=%d _seg=%d, wk2=%02x", index, offset, page, _seg, wk2);
...
void _ssd1306_pixel(SSD1306_t *dev, int xpos, int ypos, bool invert)
{
	...
	uint8_t wk1 = 1 << _bits;
	TRACE_D("ypos=%d _page=%d _bits=%d wk0=0x%02x wk1=0x%02x", ypos, _page, _bits, wk0, wk1);
	if (invert)
	{
		wk0 = wk0 & ~wk1;
	}
	...
	if (dev->_flip)
		wk0 = ssd1306_rotate_byte(wk0);
	TRACE_D("wk0=0x%02x wk1=0x%02x", wk0, wk1);
...
{
	TRACE_D("src=%02x srcBits=%d dst=%02x dstBits=%d", src, srcBits, dst, dstBits);
...
{
	TRACE_I("dev->_page[%d]._segs[%d]=%02x", page, seg, dev->_page[page]._segs[seg]);
}
